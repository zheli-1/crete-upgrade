...
void ObjectState::write_n(unsigned offset, std::vector< klee::ref<Expr> > value) {
    uint64_t NumBytes = value.size();
    for (uint64_t idx = 0; idx != NumBytes; ++idx) {
        write8(offset + idx, value[idx]);
    }
...
void ObjectState::print_refCount() 
{
    std::cerr << std::hex << "addr = 0x" << object->address
            << std::dec << ", os->refCount = " << _refCount.getCount()
            << std::endl;
}

unsigned ObjectState::get_refCount()
{
    return _refCount.getCount();
}
#endif // CRETE_CONFIG
