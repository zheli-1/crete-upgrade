...
void ExecutionState::addSymbolic(const MemoryObject *mo, const Array *array) {
  symbolics.emplace_back(std::make_pair(klee::ref<const MemoryObject>(mo), array));
...
klee::ref<Expr> ExecutionState::getConcreteExpr(klee::ref<Expr> e)
{
    klee::ref<Expr> sym_expr = constraints.simplifyExpr(e);
...
void ExecutionState::crete_assert_concolic_replay() const
{
    for(ConstraintManager::const_iterator it = constraints.begin();
        ...
        klee::ref<Expr> value = concolics.evaluate(*it);
